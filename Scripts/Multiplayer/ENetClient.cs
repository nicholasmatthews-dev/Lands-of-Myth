...
public partial class ENetClient : ENetService {
    ...
    TaskCompletionSource<bool> connectionWait = new();
    ...
    protected override void HandleResults(Godot.Collections.Array results){
        ...
        if (eventType == ENetConnection.EventType.Connect){
            if (Debugging) Debug.Print(GetType() + ": Connection established with " + peer.GetRemoteAddress());
            serverPeer = peer;
            BroadCastConnection(peer);
            connectionWait.SetResult(true);
        }
        if (eventType == ENetConnection.EventType.Receive){
            if (Debugging) Debug.Print(GetType() + ": Received packet from " + peer.GetRemoteAddress());
            Task.Run(() => {
